...
Encrypting the same message with the same key and nonce but different associated data generates distinct ciphertexts that do not reveal any additional information about the message. However, this does not imply that (nonce, key) pairs can be safely reused, even if the associated data differs.
