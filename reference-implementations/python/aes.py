"""Minimal AES forward round implementation for AEGIS reference code."""
...
GMUL2: tuple[int, ...] = _precompute(_xtime(i) for i in range(256))
GMUL3: tuple[int, ...] = _precompute(GMUL2[i] ^ i for i in range(256))
...
def _mix_single_column(s: bytearray, idx: int) -> None:
    ...
    s[idx + 2] = a0 ^ a1 ^ GMUL2[a2] ^ GMUL3[a3]
    s[idx + 3] = GMUL3[a0] ^ a1 ^ a2 ^ GMUL2[a3]
...
def aes_round(block: bytes, round_key: bytes) -> bytes:
    """Apply one forward AES round (SubBytes, ShiftRows, MixColumns, AddRoundKey).

    Args:
        block: 16-byte input state.
        round_key: 16-byte round key.

    Returns:
        16-byte output state.
    """
    if len(block) != 16 or len(round_key) != 16:
        raise ValueError("AES round expects 16-byte inputs")
